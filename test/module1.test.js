...
describe('Module 01 - Game Loop', () => {
	...
	it('Reference an external script @external-script', () => {
		const srcs = $('script')
			...
			.get();
		assert(
			srcs.includes('https://code.createjs.com/1.0.0/createjs.min.js'),
			'Did you add a `script` tag for the Createjs library?'
		);
		assert(
			srcs.includes('https://code.createjs.com/1.0.0/createjs.min.js'),
    ...
    `
			...
			'Do you have a constant called `KEYCODE_LEFT` set equal to `37`?'
		);
		assert(
			astq.query(
				ast,
    ...
    `
			...
			'Do you have a constant called `KEYCODE_UP` set equal to `38`?'
		);
		assert(
			astq.query(
				ast,
    ...
    `
			...
			'Do you have a constant called `KEYCODE_RIGHT` set equal to `39`?'
		);
		assert(
			astq.query(
				ast,
    ...
    `
		...
		assert(
			stage[0].init.callee.object.name === 'createjs' && stage[0].init.callee.property.name == 'Stage',
			'Are you using the `createjs.Stage` class?'
		);
		assert(
			stage[0].init.arguments[0].value === 'canvas',
    ...
    `
		...
		assert(
			ship[0].init.callee.object.name === 'createjs' && ship[0].init.callee.property.name == 'Shape',
			'Are you using the `createjs.Shape` class?'
		);
	});
	it('Draw the ship shape @draw-ship', () => {
		const calls = astq.query(
			ast,
			`
        // BlockStatement
          / ExpressionStatement
	...
	it('Add a shape to the stage @ship-addchild', () => {
      ...
      // CallExpression [
		...
		assert(
			child.length >= 1 && stage == 'stage' && ship == 'ship',
			'Are you adding the `ship` shape to the stage?'
		);
	});
	...
	it('Switch statement @switch-statement', () => {
		...
		const switch_variable =
			switch_statement[0].discriminant.object.name + '.' + switch_statement[0].discriminant.property.name;
		assert(
			switch_statement.length >= 1 && switch_variable == 'event.keyCode',
			'Do you have a `switch` statement that is testing `event.keyCode`?'
		);
	});
	it('Left key @left-key', () => {
		const left = astq.query(
			ast,
			`
      // SwitchCase [
